...
class SubscriptionServiceTest {
    ...
    class CreateSubscriptionTests {
        ...
        void createSubscription_forNewUser_shouldCreateSubscription() {
            ...
            when(subscriptionRepository.save(any(SubscriptionEntity.class))).thenAnswer(i -> i.getArguments()[0]);
            when(modelMapper.map(any(SubscriptionEntity.class), eq(SubscriptionDTO.class))).thenReturn(new SubscriptionDTO());
        ...
        void createSubscription_forUserWithExistingSubscription_shouldChainSubscription() {
            ...
            when(userService.getUserEntityById(1L)).thenReturn(userEntity);
            when(companyService.findCompanyEntityByOwnerId(100L)).thenReturn(companyEntity);
            when(subscriptionRepository.isAvailableYet(anyLong(), any(LocalDate.class))).thenReturn(lastEndDate);
            when(subscriptionRepository.save(any(SubscriptionEntity.class))).thenAnswer(i -> i.getArguments()[0]);
            when(modelMapper.map(any(SubscriptionEntity.class), eq(SubscriptionDTO.class))).thenReturn(new SubscriptionDTO());
        ...
        void createSubscription_withZeroDuration_shouldThrowConflictException() {
            assertThrows(ConflictException.class, () -> subscriptionService.createSubscription(1L, LocalDate.now(), 0, 100L));
        }
        ...
        void createSubscription_whenUserHasNoCompany_shouldThrowConflictException() {
            ...
            assertThrows(ConflictException.class, () -> subscriptionService.createSubscription(1L, LocalDate.now(), 1, 100L));
        }
    ...
    class GetSubscriptionsByUserIdTests {
        ...
        void getSubscriptionsByUserId_whenUserIsValid_shouldReturnSubscriptions() {
            when(userService.getUserEntityById(1L)).thenReturn(userEntity);
            when(companyService.findCompanyEntityByOwnerId(100L)).thenReturn(companyEntity);
            when(subscriptionRepository.findByUserIdOrderByStartDateDesc(1L)).thenReturn(Collections.singletonList(new SubscriptionEntity()));
        ...
        void getSubscriptionsByUserId_whenUserIsNotRoleUser_shouldThrowConflictException() {
            ...
            assertThrows(ConflictException.class, () -> subscriptionService.getSubscriptionsByUserId(1L, 100L));
        }
    }
    ...
    class DisableSubscriptionTests {
        ...
        void disableSubscription_whenSubscriptionsExist_shouldSetInactive() {
            ...
            when(subscriptionRepository.disableSubscriptionByCompanyAndUserId(10L, 1L)).thenReturn(List.of(1L, 2L));
