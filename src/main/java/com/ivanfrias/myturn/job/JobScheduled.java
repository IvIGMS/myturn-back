...
public class JobScheduled {
  ...
  public void createNotifications() {
    ...
    Map<Long, List<UserDownDto>> userByCompany =
        userDownToday.stream().collect(Collectors.groupingBy(UserDownDto::getCompanyId));

    userByCompany.forEach(
        (key, values) -> {
  ...
  private NotificationDto createNotificationStructure(UserDTO owner, List<UserDownDto> values) {
    ...
    message
        .append("Hola " + owner.getFirstname() + " " + owner.getLastname() + "\n")
        .append("Ha finalizado la suscripción de los siguientes usuarios:\n");

    values.forEach(user -> message.append(user.getName() + " " + user.getLastName() + "\n"));
  ...
  public void sendNotifications() {
    List<NotificationEntity> notifications =
        notificationService.getNotificationsNotSendedByType(NotificationTypeEnum.END_SUBSCRIPTION);
