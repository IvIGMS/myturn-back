...
public class NotificationService {
    ...
    public List<NotificationEntity> getNotificationsNotSendedByType(NotificationTypeEnum notificationTypeEnum) {
    ...
    public void sendNotifications(List<NotificationEntity> notifications) {
        notifications.forEach(n -> {
            if(!appProps.mail().isEmailMocked()) {
                ...
                log.warn("[EMAIL MOCKED] No se envía email real. to={}, subject={}", n.getEmailTo(), n.getSubject());
                n.setIsSended(true); // Le ponemos isSended aunque no se envíe para que la app actue igual
            }
        });
    }
}
