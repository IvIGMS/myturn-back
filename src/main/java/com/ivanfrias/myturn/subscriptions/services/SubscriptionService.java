...
public class SubscriptionService {
  ...
  public SubscriptionDTO createSubscription(
      Long userId, LocalDate startDate, int durationInMonths, Long ownerId) {
  ...
  public List<SubscriptionDTO> getSubscriptionsByUserId(Long userId, Long ownerId) {
    ...
    if (!user.getRole().equals(RoleEnum.USER)) {
      throw new ConflictException(
          "No se pueden ver las suscripciones de un user que no sea role USER");
    }
    isLinkedOwnerAndUser(userId, ownerId);
    List<SubscriptionEntity> subscriptions =
        subscriptionRepository.findByUserIdOrderByStartDateDesc(user.getId());
  ...
  public SubscriptionDTO getSubscriptionById(Long subscriptionId) {
    SubscriptionEntity subscription =
        subscriptionRepository
            ...
            .orElseThrow(
                () -> new NotFoundException("Suscripción no encontrada con ID: " + subscriptionId));
  ...
  private void isLinkedOwnerAndUser(Long userId, Long ownerId) {
    ...
    if (Objects.isNull(user.getCompany())) {
      throw new ConflictException(
          "El usuario para el que se va a crear una suscripción aún no tiene una empresa asignada");
    }

    if (!user.getCompany().getId().equals(ownerCompany.getId())) {
      throw new ConflictException(
          "El usuario no pertenece a la misma empresa que el administrador, no tiene permisos para crear suscripciones");
    }
  }
  ...
  public void disableSubscription(Long companyId, Long userId) {
    List<Long> idList =
        subscriptionRepository.disableSubscriptionByCompanyAndUserId(companyId, userId);
            ...
            id -> {
              SubscriptionEntity subscription =
                  subscriptionRepository
                      .findById(id)
                      .orElseThrow(
                          () -> new NotFoundException("Subscription not found: " + companyId));
